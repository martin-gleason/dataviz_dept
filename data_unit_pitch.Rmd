...
* This unit would collapse existing resources to:
  ...
  * Prepare for [CFive's Supervisor](https://www.cfive.com/products/supervisor/)'s data collection and dashboards features
...
.pull-right[![:scale 85%](imgs/Agile-team.jpg)]
...
* Redesign reports for
  ...
  * Portability
      * *Share-able regardless of format*
...
The remaining seven columns contain the PO's case load data.
---
...
The remaining columns are a combination of unit, division, and department wide data used for baseline comparison.
...
.center[![:scale 55%](imgs/tidy_data1.png)]
  ...
  mutate(X__1 = cell_spec(1, 
         ...
         X__8 = cell_spec(8,
                          popover = spec_popover(content = "Blank column to force formatting",
                                                 position = "top")),
         X__9 = cell_spec(9,
                          popover = spec_popover(content = "Caseload Total should be computed number on a separate page.",
                                                 position = "top")),
         X__10 = cell_spec(10,
...
.pull-right[![](imgs/sofasogood_ron.gif)]
</br>
</br>

## Adding an additional unit makes the data twice as messy.
---
## GBO and Lawndale
...
* Specific stats are duplicated throughout the table
  * Repetition of each variable makes it difficult to read
  
* Intra-unit stats can be read
  * It is easy to get lost in the columns
...
.pull-right[
### Pros
* Tidy format allows for faster selection of specific variables
* Filtering for specific observations is faster
* Computed variables (average, total number of POs) will be done by the software
]

.footnote[1: The columns are: *`r names(cal_55_2 %>% tidy_up()) %>% as.vector()`*]
---
## New Format: Visualizations
* *Tidy format is built to be analyzed, not presented as 17 column reports*
...
* This table focuses only on case load and risk levels
  * It is the same data as the 17 column table, with an additional 5 lines of code
...
* Unlike the Excel format, this document calculates these totals and averages based on input

* This representation is concise and easy to read
...
* R is a free scripting language
  * Built for analysis
  * Meaning it can be executed with new data instead of recreated as a spreadsheet
* Including: 
...
.left-code[
```{r SI_VOP, include= TRUE, echo = TRUE, eval = FALSE, dpi = 300, fig.cap="Socials and VOPs in GBO and Lawndale"}
...
.right-plot[
```{r SI_VOP_out, ref.label="SI_VOP", echo=FALSE, eval=TRUE}
```
]
---
# Code, Tables, Graphs
...
---
```{r, markham_north, include = TRUE, echo = FALSE, fig.cap = "Socials and VOPs in Markham North", fig.align='center', fig.pos = "h", out.height='85%', out.width='85%', dpi = 600}
...
* `R` allows for code and report to be included in the same document
  * No need to cut and paste, or push to PDF or Word
...
* A unit with a dedication mission will be able to
